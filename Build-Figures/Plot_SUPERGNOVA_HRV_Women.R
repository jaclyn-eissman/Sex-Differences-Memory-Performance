...
GLOBALRES = fread(paste0(dir,"Women_SDNN_GLOBALRES.txt"))
GLOBALRES$Phenotype = "Resilience"
memslopes = fread(paste0(dir,"Women_SDNN_memslopes.txt"))
memslopes$Phenotype = "Memory_Decline"
Data = rbind(GLOBALRES,memslopes)

pdf(paste0(dir,"Women_HRV_BP_vs_Rho_by_CHR_ALL.pdf"),width=13,height=7)
...
pdf(paste0(dir,"Women_HRV_BP_vs_Rho_SigReg.pdf"),width=13,height=7)
...
pdf(paste0(dir,"Women_HRV_BP_vs_Rho_Sig_memslopes.pdf"),width=13,height=7)
print(ggplot(Data_Sig, aes(x=start, y=rho, color=Phenotype)) + geom_line() + theme_bw()+ 
        theme(axis.text.x=element_text(angle=45, hjust=1)) + theme(legend.position="top") +
        ylab("Covariance") + xlab("Start of Region"))
dev.off()
